...
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Create Scholarship</h1>
        <p class="text-gray-600 mt-2">Define scholarship criteria and benefits</p>
    </div>
        ...
        <form action="{{ route('admin.scholarships.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" required>{{ old('description') }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="application_deadline" value="{{ old('application_deadline') }}"
                           class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" required>
                    @error('application_deadline') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <input type="text" name="criteria[economic_status]" placeholder="Economic Status"
                           value="{{ old('criteria.economic_status') }}"
                           class="border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                    <input type="text" name="criteria[major]" placeholder="Major"
                           value="{{ old('criteria.major') }}"
                           class="border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                </div>
                @error('criteria') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="flex items-center justify-end space-x-3">
                <a href="{{ route('admin.scholarships.index') }}" class="px-4 py-2 border rounded-lg text-gray-700">Cancel</a>
